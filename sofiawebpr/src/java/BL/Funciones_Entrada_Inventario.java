...
public class Funciones_Entrada_Inventario extends DA.consultas_Entrada_Inventario{
   ...
   public Object descontarCantidad(Object id_producto_inventario,float cantidad,Object motivo, int id_usuario, int idSucursal)
   {
       ...
       consultaRegistrarDescuentoProducto(id_producto_inventario, motivo,cantidad,id_usuario);
       general.registrarHistorial("descontarCantidad", id_usuario, date.format(now),hora.format(now), "Se Actualizo producto en inventario ID:"+id_producto_inventario+"");           
       return consecutivo;
   }
...
//       return gr.CargarGrd(consultaBuscarProductoInventario(nombre,opcion,id_sucursal), nombreColumnas, ancho);
 ...
 public Object eliminarProductoInventario(Object id_producto_inventario, 
                                            Object idMotivo,
                                            int idSucursal, 
                                            int idUsuario)
  {
      ...
      Object horaNow , fecha;
      fecha = date.format(now);
      horaNow = hora.format(now);
      Object consecutivo =general.consultaConsecutivo(idSucursal);
      if(consultaEliminarProductoInventario(id_producto_inventario))
      {
          ...
          con_gen.registrarHistorial("eliminarProductoInventario", idUsuario, fecha, horaNow, descripcion);
          consultaRegistrarProductoEliminado(id_producto_inventario, descripcion, idUsuario, idMotivo);
      }
      return consecutivo;
  }
  public ArrayList motivoEliminacionCombo()
  {
       try {
            rh = consultaMotivosEliminacion();
            return general.resultSetToArrayList(rh);
        } catch (Exception e) {
            e.printStackTrace();
            return null;
