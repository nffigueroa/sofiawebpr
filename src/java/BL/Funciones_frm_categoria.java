...
//       return gr.CargarGrd(llenarTabla_Categoria(), nombreColumnas, ancho);
     ...
     public ArrayList llenarTabaCategoriaRelacion(int idSucursal)
   {
          try {
            rs = consultaRelacionSucursalCategoria(idSucursal);
            return general.resultSetToArrayList(rs);
        } catch (SQLException ex) {
            Logger.getLogger(Funciones_frm_producto.class.getName()).log(Level.SEVERE, null, ex);
            return null;
...
//       return gr.CargarGrd(llenarTabla_Categoria(), nombreColumnas, ancho);
    ...
    public ArrayList InsertarRelacionCategoriaSucursal(JSONObject categoria,int idSucursal)
    {
        //categoria = categoria.toUpperCase();
        Consultas_Generales con_generales = new Consultas_Generales();
        con_generales.registrarHistorial("InsertarRelacionCategoriaSucursal", idSucursal, date.format(now),hora.format(now), "Se registra Relacion de categoria "+categoria+"");
         try {
            rs = consultaRegistrarRelacionSucursalCategoria(categoria,idSucursal);
            return general.resultSetToArrayList(rs);
        } catch (SQLException ex) {
            Logger.getLogger(Funciones_frm_producto.class.getName()).log(Level.SEVERE, null, ex);
            return null;
