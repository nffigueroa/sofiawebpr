 ...
 * @author Nestor1
 */
public class Funciones_frm_MasVendido extends DA.Consultas_informeMasVendido{
    ...
    public ArrayList llenarTablaMasVendido(int id_sucur, String fechaIni, String fechaFin){
        try{
            return general.resultSetToArrayList(consultaProductoVendido(id_sucur,fechaIni,fechaFin));
        }
        catch(Exception e) {
            e.printStackTrace();
            return null;
    ...
    public  Constructores.Constructo_Cantidad_Productos_Vendido llenarMasVendido(int id_sucur, String fechaIni, String fechaFin)
   {
       int aux =0;
       ...
       float[] iva = new float[id_producto.length];
       float[] descuento = new float[id_producto.length];
       float[] efectivo = new float[id_producto.length];
       float[] credito = new float[id_producto.length];
       float[] totalVenta = new float[id_producto.length];
       float[] subTotal = new float[id_producto.length];
       ResultSet rh = null;
       rh = consultaProductoVendido(id_sucur,fechaIni,fechaFin);
         try {
             while(rh.next())
             {
                 for (int i = 0; i < id_producto.length; i++) {
                    ...
                    if(rh.getInt("INVEN.id_producto_inventario")==id_producto[i])
                    {
                        ...
                        iva[i] = iva[i] + rh.getFloat("FACT.iva");
                        descuento[i] = descuento[i] + rh.getFloat("FACT.descuento");
                        efectivo[i] = efectivo[i] + rh.getFloat("FACT.efectivo");
                        if(rh.getString("INVEN.id_producto_inventario").equals("TARJETA CREDITO"))
                        {
                            credito[i] = credito[i] + rh.getFloat("FACT.efectivo");
                        }
                    }
                 }
